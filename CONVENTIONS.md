...
- An app with no network code blocks `android.permission.INTERNET` and the storage and
  media permissions no module needs, and carries `plugins/withDebugInternet.js` so a
  development build can still load its bundle.
