...
Expo Go is enough for development: the camera, microphone and photo picker all work there. In Settings, enter your server's URL (on a LAN, the app pre‑fills the Metro host with port 8787). For a store build use `npx expo prebuild` + `expo run:ios` / `expo run:android`, or EAS Build.
